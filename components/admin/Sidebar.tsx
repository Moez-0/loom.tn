...
export default function AdminSidebar({ sectionLabel, links }: AdminSidebarProps) {
        ...
        {links.map((link) => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`block rounded-md border px-4 py-3 text-[0.813rem] uppercase tracking-[0.06em] transition ${
                isActive
                  ? 'border-loom-accent bg-loom-accent/15 text-white'
                  : 'border-loom-border bg-loom-surface text-loom-muted hover:border-loom-accent hover:text-white'
              }`}
            >
              {link.label}
            </Link>
